...
const calculateMedian = (values: (number | string)[]): number => {
    ...
    const numericValues = values.map(value => typeof value === 'string' ? parseFloat(value) : value);
    ...
    if (numericValues.length % 2 === 0) {
        ...
        return Number(((numericValues[middleIndex - 1] + numericValues[middleIndex]) / 2).toFixed(3));
    } else {
...
const calculateMode = (values: (number | string)[]) => {
    ...
    numericValues.forEach(single => {
        ...
        //If frequency of current element is more than maxFreq, then update
...
const calculateByClass = (data: wineData[], gamma: boolean = false) => {
    ...
    data.forEach(item => {
        ...
        const parameterForStat: number | string = gamma ? item.Gamma as number : item.Flavanoids;
...
const addGamma = (data: wineData[]) => {
    return data.map(point => {
        ...
        const gamma = (Number(point.Ash) * Number(point.Hue)) / Number(point.Magnesium);
...
export { calculateMean, calculateMedian, calculateMode, calculateByClass, addGamma };
